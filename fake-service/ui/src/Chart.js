...
import {ReactComponent as HappyOperatorSvg} from "./imgs/happy-operator.svg"
import {ReactComponent as UnhappyOperatorSvg} from "./imgs/unhappy-operator.svg"
import {ReactComponent as HappyPManagerSvg} from "./imgs/happy-pmanager.svg"
import {ReactComponent as UnhappyPManagerSvg} from "./imgs/unhappy-pmanager.svg"
import {ReactComponent as HappyEngineerSvg} from "./imgs/happy-engineer.svg"
import {ReactComponent as UnhappyEngineerSvg} from "./imgs/unhappy-engineer.svg"
import {ReactComponent as HappySecuritySvg} from "./imgs/happy-security.svg"
import {ReactComponent as UnhappySecuritySvg} from "./imgs/unhappy-security.svg"
...
const NodeInnerCustom = ({node, children, ...otherProps}) => {
    ...
    const selectCharacter = (owner, responseCode) => {
        if (owner === "oli") {
            return responseCode < 400 ? <HappyOperatorSvg className="floating-svg"/> :
                <UnhappyOperatorSvg className="floating-svg"/>
        }
        if (owner === "pam") {
            return responseCode < 400 ? <HappyPManagerSvg className="floating-svg"/> :
                <UnhappyPManagerSvg className="floating-svg"/>
        }
        if (owner === "sam") {
            return responseCode < 400 ? <HappySecuritySvg className="floating-svg"/> :
                <UnhappySecuritySvg className="floating-svg"/>
        }
        if (owner === "dave") {
            return responseCode < 400 ? <HappyEngineerSvg className="floating-svg"/> :
                <UnhappyEngineerSvg className="floating-svg"/>
        }
    ...
    return (
        <Container {...otherProps} className={className} key={node.properties.name}>
            {selectCharacter(node.properties.owner, node.properties.response)}
            <Row>
